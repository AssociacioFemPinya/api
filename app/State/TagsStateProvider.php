...
final class TagsStateProvider extends AbstractStateProvider
{
    ...
    protected function collectionProvider(Operation $operation, array $uriVariables = [], array $context = []) : mixed
    {
        ...
        if(!is_null($this->casteller)) $tags->where('colla_id',$this->casteller->getColla());

        if(array_key_exists('type',$this->parameters)) $tags->where('type',(string)$this->parameters['type']['value']);
